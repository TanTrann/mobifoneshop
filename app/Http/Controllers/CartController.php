...
public function add_cart_ajax_sim(Request $request){
    // Session::forget('cart');
$data = $request->all();
$session_id = substr(md5(microtime()),rand(0,26),5);
$cart = Session::get('cart');
if($cart==true){
    $is_avaiable = 0;
    foreach($cart as $key => $val){
        if($val['sim_id']==$data['cart_sim_id']){
            $is_avaiable++;
        }
    }
    if($is_avaiable == 0){
        $cart[] = array(
            'session_id' => $session_id,
            'sim_number' => $data['cart_sim_number'],
            'sim_id' => $data['cart_sim_id'],
            'sim_image' => $data['cart_sim_image'],
            'sim_quantity' => $data['cart_sim_quantity'],
            'sim_qty' => $data['cart_sim_qty'],
            'sim_price' => $data['cart_sim_price'],
        );
        Session::put('cart',$cart);
    }
}else{
